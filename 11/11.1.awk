...
END {
    ...
    for (tree = 0; tree < length(trees); tree++) {
        split("", previous_space, "")
        
        previous_space[0][0] = "00"

        for (year = 1; year <= 100; year++) {
            for (y in previous_space) {
                for (x in previous_space[y]) {
                    id = previous_space[y][x]
                    ...
                    for (d in dirs) {
                        ...
                        if (dy in previous_space && dx in previous_space[dy] && previous_space[dy][dx] == "#") {
                            continue
                        }
